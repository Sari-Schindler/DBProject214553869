 ...
 **הגדרת בסיס הנתונים:**
  ...
  <li><strong>ניהול הגדרות:</strong> כל פרמטרי החיבור (Host, Port, DB Name) מנוהלים וניתנים לעדכון בקובץ <code>database.py</code>.</li>
</ul>.
...
<ol>
  <li><strong>בדיקת זמינות השרת:</strong> לפני הרצת האפליקציה, יש לוודא ששרת בסיס הנתונים (PostgreSQL) פעיל ורץ.</li>
  <li><strong>פרמטרי החיבור:</strong> פרמטרים אלו מוגדרים ומנוהלים בקובץ <code>database.py</code>. להלן פרטי ברירת המחדל המוגדרים במערכת:
    <ul>
      <li><strong>כתובת מארח (Host):</strong> 127.0.0.1 </li>
      <li><strong>פורט (Port):</strong> 5433 </li>
      <li><strong>שם בסיס הנתונים:</strong> MusicLesson </li>
      <li><strong>שם משתמש:</strong> postgres </li>
      <li><strong>סיסמה:</strong> Mss054333@ </li>
    </ul>
  </li>
</ol>
...
<ul>
  <li><strong>תפריט ראשי:</strong> מציע 5 אפשרויות ניווט לניהול תלמידים, שיעורים, רישום מאובטח ודוחות.
    <br>
    <img src="part5/screenshots/main.png" alt="תפריט ראשי" width="600">
  </li>
  
  <li><strong>מסך ניהול תלמידים:</strong> במסך זה ניתן לבצע שליפה, הוספה, עדכון ומחיקה של תלמידים.
    <br>
    <img src="part5/screenshots/students.png" alt="ניהול תלמידים" width="600">
  </li>

  <li><strong>מסך ניהול שיעורים:</strong> מאפשר ניהול CRUD מלא של קטלוג השיעורים ועדכון מחירים.
    <br>
    <img src="part5/screenshots/lessons.png" alt="ניהול שיעורים" width="600">
  </li>

  <li><strong>מסך ניהול הרשמה:</strong> הזנת SID ו-LID להפעלת הפרוצדורה המאובטחת והצגת הודעות חיווי מהשרת.
    <br>
...
<ul>
  <li>גישה למאגר התלמידים המלא ותצוגתו בטבלה אינטראקטיבית.</li>
  <li>הוספת תלמידים חדשים למערכת על ידי הזנת שם וכתובת.
    <br>
    <img src="part5/screenshots/adds.png" alt="הוספת תלמיד" width="600">
  </li>
  <li>עדכון פרטי תלמידים קיימים ומחיקת רשומות מהמסד לאחר בחירת שורה בטבלה.
    <br>
...
<ul>
  ...
  <li>המערכת משתמשת ב-Explicit Cursor וב-Loop בתוך ה-SQL כדי לסרוק את היסטוריית התלמיד ולמנוע כפל קורסים.</li>
  <li>קבלת הודעות חיווי (Notices) ושגיאות טריגרים (כמו חדר מלא) ישירות בחלונות קופצים (Message Boxes).
    <br>
...
<ul>
  <li>הפקת דוחות ניהוליים המבוססים על השאילתות שנוצרו בשלבי התכנון המוקדמים.</li>
...
#### תהליך הפיתוח
<ul>
  <li><strong>1. אינטגרציה עם בסיס הנתונים:</strong> חיבור האפליקציה השולחנית לבסיס הנתונים הקיים מהשלבים הקודמים באמצעות ספריית <code>psycopg2</code>.</li>
  <li><strong>2. עיצוב מודולרי:</strong> פיצול הקוד למודולים נפרדים (תיקיית <code>screens</code>) כדי להבטיח קוד נקי, הפרדת סמכויות ותחזוקתיות קלה של המערכת.</li>
  <li><strong>3. עיצוב ממשק משתמש:</strong> יישום ממשק מקצועי ונקי תוך שימוש ב-Styles אחידים וערכות צבעים המעידות על סוג הפעולה (ירוק להצלחה, אדום למחיקה).</li>
  <li><strong>4. טיפול בשגיאות:</strong> הטמעת מנגנוני <code>try-except</code> ותפיסת הודעות <code>RAISE NOTICE</code> מהפרוצדורות ב-SQL כדי לספק משוב מפורט ובזמן אמת למשתמש.</li>
</ul>

#### תכונות מרכזיות שהוטמעו
<ul>
  <li><strong>פעולות CRUD מלאות:</strong> אפשרות לביצוע שליפה, הוספה, עדכון ומחיקה עבור לפחות 3 ישויות מרכזיות בבסיס הנתונים.</li>
  <li><strong>תצוגת נתונים בזמן אמת:</strong> חיבור חי וישיר למסד הנתונים המבטיח עדכון מיידי של הטבלאות הגרפיות לאחר כל שינוי.</li>
