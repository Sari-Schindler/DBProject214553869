...
פרוצדורה זו מבצעת רישום מאובטח של תלמיד לשיעור תוך שימוש בלוגיקה עסקית מתקדמת, ומבטיחה שקיפות מלאה למשתמש ושמירה על שלמות הנתונים:
                                                                    ...
                                                                    -            Explicit Cursor (ניהול ידני): הוטמע קורסור מפורש (cursor_student_courses) המאפשר שליטה מלאה על שליפת הנתונים מהזיכרון, מעבר לשימוש בשאילתות פשוטות.

- מבנה לולאה (Loop & Fetch): שימוש במבנה ידני של OPEN, FETCH ו-CLOSE. המערכת סורקת שורה-שורה את היסטוריית הלמידה של התלמיד ומבצעת השוואה לוגית מורכבת בזמן אמת.

- לוגיקה השוואתית: הפרוצדורה לא רק בודקת כפל רישום טכני, אלא מזהה התנגשויות פדגוגיות (כגון ניסיון רישום לקורס נוסף באותו סוג כלי נגינה) ומציגה התראות (NOTICES) מפורטות.

- טיפול בחריגות (Exception Handling): שימוש בבלוק EXCEPTION לתפיסת שגיאות מסד נתונים (כגון הפרת אילוצי ייחודיות) והמרתן להודעות ידידותיות למשתמש בממשק הגרפי..
...
#### 👥 ניהול תלמידים
- **ניהול רשומות:** אפשרות לצפייה בכל התלמידים, הוספת תלמידים חדשים, עדכון פרטים ומחיקת רשומות ישירות מהממשק.
- **עדכון בזמן אמת:** הטבלה מתרעננת אוטומטית לאחר כל פעולה כדי להציג את המידע העדכני ביותר.

#### 🎼 ניהול שיעורים
- **ניהול מחירים וסוגים:** צפייה בשיעורי המוזיקה הקיימים ועדכון מחירים בצורה פשוטה ומהירה.

#### 🔐 רישום מאובטח ולוגיקה עסקית

- **ביצוע רישום:** הפעלת הפרוצדורה pr_SafeRegister המבצעת בדיקות בטיחות (מניעת כפל רישום ובדיקת תפוסת חדרים).

- **מחשבון הנחות:** הפעלת הפונקציה fn_CheckDiscount לחישוב אחוז ההנחה המגיע לתלמיד לפי רמת פעילותו.

- **חיווי למשתמש:** המערכת מציגה הודעות NOTICE ושגיאות טריגרים ישירות בחלונות קופצים (לדוגמה: אזהרה כשהחדר מלא).
...
**1. אינטגרציה עם בסיס הנתונים:** חיבור האפליקציה לבסיס הנתונים הקיים משלבים קודמים באמצעות psycopg2.

**2. עיצוב מודולרי:** פיצול הקוד למודולים נפרדים (תיקיית screens) כדי להבטיח קוד נקי ותחזוקתיות קלה.

**3. עיצוב ממשק משתמש:** יישום ממשק מקצועי ונקי תוך שימוש ב-Styles אחידים וצבעים המעידים על סוג הפעולה (ירוק להצלחה, אדום למחיקה).

**4. טיפול בשגיאות:** הטמעת מנגנוני try-except ותפיסת הודעות RAISE NOTICE מהפרוצדורות ב-SQL כדי לספק משוב בזמן אמת למשתמש.

#### תכונות מרכזיות שהוטמעו

-פעולות CRUD מלאות: יצירה, שליפה, עדכון ומחיקה עבור 3 ישויות בסיס נתונים לפחות

-תצוגת נתונים בזמן אמת: חיבור חי למסד הנתונים עם עדכון מיידי של הטבלאות הגרפיות 

-מניעת טעויות: אימות קלט והודעות שגיאה ידידותיות במקרה של הפרת אילוצים (כמו חדר מלא) 
